...
class MultimodalModel(nn.Module):
    ...
    def forward(self, image, text_metadata):
        ...
        # image_pooled = image_features.mean(dim=1)  # (batch, common_dim)
        # text_pooled = text_features.mean(dim=1)    # (batch, common_dim)
        ...
        ## Bloco residual
        ## image_features_residual_after_cross_attention = self.image_residual(image_features, image_features_att, image_features_att)
        ## text_features_att = self.text_residual(text_features, text_features_att, text_features_att)
        
        image_features_residual_after_intra_attention = self.image_residual(image_features, image_features_att, image_features_att)
        text_features_residual_after_intra_attention = self.image_residual(text_features, text_features_att, text_features_att)
        ...
        # # text_features_residual_after_cross_att, _ = self.text_cross_attention(
        # #     query=text_features_att,
        # #     key=image_features_residual_after_cross_attention,
        # #     value=image_features_residual_after_cross_attention
        # # )

        # Bloco residual após usar o Cross-Attention    
        # image_features_residual_after_cross_attention = self.image_residual(image_features_att, image_features_att, image_features_att)
        # text_features_residual_after_cross_attention = self.text_residual(text_features_att, text_cross_att, text_cross_att)


        # # Self-attention após o residual   
        # image_features_residual_after_cross_attention, _ = self.image_self_attention(
        #     image_features_att, image_features_att, image_features_att
        # )

        # text_features_residual_after_cross_attention, _ = self.text_self_attention(
        #     text_cross_att, text_cross_att, text_cross_att
        # )

        # # Bloco residual após usar o Cross-Attention e juntar com as features projetadas
        # image_features_residual_after_cross_attention = self.image_residual(image_features, image_features_att, image_features_att)
        # text_features_residual_after_cross_attention = self.text_residual(text_features, text_features_residual_after_cross_attention, text_features_residual_after_cross_attention)

        # # === [E] Pooling das atenções finais 
        # image_features_residual_after_cross_attention = image_features_residual_after_cross_attention.unsqueeze(0)  
        # text_features_residual_after_cross_att = text_features_residual_after_cross_att.unsqueeze(0) 
        ...
        # text_features_residual_after_cross_attention = text_features_residual_after_cross_att.permute(1, 0, 2)    # (batch, seq_len_text, common_dim)
        ...
        # === [E] Pooling das atenções finais 
        image_features_residual_after_intra_attention = image_features_residual_after_intra_attention.unsqueeze(0)  
        text_features_residual_after_intra_attention = text_features_residual_after_intra_attention.unsqueeze(0) 
        image_features_residual_after_intra_attention = image_features_residual_after_intra_attention.permute(1, 0, 2)  # (batch, seq_len_img, common_dim)
        text_features_residual_after_intra_attention = text_features_residual_after_intra_attention.permute(1, 0, 2)    # (batch, seq_len_text, common_dim)

        
        image_pooled = image_features_residual_after_intra_attention.mean(dim=1)  # (batch, common_dim)
        text_pooled = text_features_residual_after_intra_attention.mean(dim=1)    # (batch, common_dim)
        ...
        if self.attention_mecanism=="no-metadata":
            ...
            if self.cnn_model_name in ["google/vit-base-patch16-224","openai/clip-vit-base-patch16", "facebookresearch/dinov2"]:
                # Os modelos ViT possuem uma sequência de tokens que precisa ser processada antes de ser projetada
                projected_image_features = image_pooled.view(b_i, s_i, -1).mean(dim=1)  # (batch, common_dim)
                projected_text_features = text_pooled.view(b_tt, s_tt, -1).mean(dim=1)  # (batch, common_dim)
